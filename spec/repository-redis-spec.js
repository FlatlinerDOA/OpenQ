...
describe('When creating a new Redis repo, ', function () {
    ...
    describe('When writing a new message with expected qid of -1 (any), ', function () {
        ...
        describe('When writing a second message with an expected version of -1 (any), ', function () {
            var error = null;
            repo.write('urn:test', { type: 'urn:test', messageNumber: 2 }, Qid.ExpectAny, function (err) {
                error = err;
            });
        ...
        describe('When writing a second message with an expected version of 0, ', function () {
            ...
            repo.write('urn:test', { type: 'urn:test', messageNumber: 2 }, 0, function (err) {
                error = err;
            });
...
describe('When creating a new Redis repo, ', function () {
    ...
    describe('When writing three messages, ', function () {
        var error;
        for (var i = 0; i < 3; i++) {
            repo.write('urn:test', { type: 'urn:test', messageNumber: i + 1 }, Qid.ExpectAny, function (err) {
                error = err;
            });
        }
